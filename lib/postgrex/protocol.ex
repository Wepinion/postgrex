defmodule Postgrex.Protocol do
  ...
  defp recv_ready({:ok, %{transactions: :strict} = s}, status, buffer) do
    # Handle strict transactions with unwrapped state
    recv_strict_ready(s, status, s.postgres, buffer)
  end
        ...
        case parse_aurora_dsql_error(error_buffer) do
          {:ok, error_msg} ->
            ...
            error = %Postgrex.Error{
              ...
              postgres: %{
                severity: "ERROR",
                code: :feature_not_supported, pg_code: "0A000",
                message: error_msg
              }
            }
